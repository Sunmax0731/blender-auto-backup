...
4. `Backup Now` で代表動作を確認します。
5. `Start Auto Backup` で定期バックアップを開始します。

保存先を空にした場合、`Source Folder\.blender-auto-backup` に ZIP が作成されます。
...
- 大容量フォルダの ZIP 作成は Blender UI thread 上で動くため、非常に大きい作業フォルダでは操作が重くなる可能性があります。
