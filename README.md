...
- ZIP 作成前に対象ファイル数、合計バイト数、保存予定先を確認する Preview
- 手動バックアップ実行
...
1. `npm test` を実行して `dist/blender-auto-backup-0.1.1.zip` を生成します。
2. Blender を起動します。
...
5. `Preview Backup` で対象ファイル数、合計バイト数、保存予定先を確認します。
6. `Backup Now` で代表動作を確認します。
7. `Start Auto Backup` で定期バックアップを開始します。
...
- `dist/blender-auto-backup-0.1.1.zip`
- `dist/blender-auto-backup-docs-0.1.1.zip`
